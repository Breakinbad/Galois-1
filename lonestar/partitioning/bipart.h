...
public:
  ...
  std::vector<double> EMvec = std::vector<double>(200);
  void initPartition() { 
    pd.locked = false;
  }
...
struct partInfo {
  ...
  explicit partInfo(unsigned mw) : partNum(0), partWeight(mw) {}

  partInfo() : partNum(0),  partWeight(0) {}

  partInfo(unsigned pn, unsigned pw)
      : partNum(pn), partWeight(pw) {}

...
void RI_f(GNode node, GGraph* fineGGraph) {
  unsigned i = 0;
  unsigned ii = 0;
  for (auto n : fineGGraph->edges(node)) { //nodes
     ii++;
     auto tmp = fineGGraph->getEdgeDst(n);
     ii += fineGGraph->getData(tmp).getWeight();
     int ss = std::distance(fineGGraph->edge_begin(tmp), fineGGraph->edge_end(tmp));
     ii += ss;
  }
  fineGGraph->getData(node).netval = ii;//*(MAX+1)+ fineGGraph->getData(node).netrand;
}
